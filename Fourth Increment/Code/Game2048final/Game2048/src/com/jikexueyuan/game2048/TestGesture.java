...
public void	train() {
	...
	while(!exception3){
	try{
    OpdfMultiGaussianFactory initFactoryupdown = new OpdfMultiGaussianFactory(
            3);

    Reader learnReaderupdown = new FileReader(new File (myDir, "punchlearn.seq"));
    List<List<ObservationVector>> learnSequencesupdown = ObservationSequencesReader
            .readSequences(new ObservationVectorReader(), learnReaderupdown);
    learnReaderupdown.close();

    KMeansLearner<ObservationVector> kMeansLearnerupdown = new KMeansLearner<ObservationVector>(
            x3, initFactoryupdown, learnSequencesupdown);
    // Create an estimation of the HMM (initHmm) using one iteration of the
    // k-Means algorithm
    Hmm<ObservationVector> initHmmupdown = kMeansLearnerupdown.iterate();

    // Use BaumWelchLearner to create the HMM (learntHmm) from initHmm
    BaumWelchLearner baumWelchLearnerupdown = new BaumWelchLearner();
     this.learntHmmupdown = baumWelchLearnerupdown.learn(
            initHmmupdown, learnSequencesupdown);
     exception3=true;
